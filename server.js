    ...
    return new Promise((resolve,reject)=> {
        db.query(sqls,[msg_ids,others,mine_id],function(err,res){ 
            if(err){
                ...
                // console.log(msg_ids+','+others+','+mine_id)
            }          
...
app.get('/profile_page',async(req,res)=>{
    var Name=  await lib.fetchUserName(req.session.user_id);
    user_picture= await usersImages(req.session.user_id);
    res.render('profile_settings.ejs',{Name:Name,user_picture:user_picture,sessioned_id:req.session.user_id,servermain:servermain});
})
/* end here */
var main_user='';
...
app.post('/request', async(req,res) =>{
    ...
    var ExcptMe=await lib.AllUsers(req.body.mine_id);    
    // for(const excpme of ExcptMe){
    //     console.log(excpme.user_id);
    // }
        for(const excpme of ExcptMe){                        
            var msgsdisplay=await Homemessagess(req.body.mine_id,excpme.user_id);            
            pictures=await usersImages(excpme.user_id);            
            if(msgsdisplay.user_id!=0){
...
var Oldpassword=await lib.oldpass(req.session.user_id);
    res.render('change_password.ejs',{Oldpassword:Oldpassword,servermain:servermain});
    })
...
app.post('/updatepass',async(req,reults)=>{
    var newPass=req.body.newPass;
    db.query("update users set password= ? where user_id=?",[newPass,req.session.user_id],function(err,res){
        if(err){
...
socket.on('store',function(data){
    ...
    socket.on('send_message', async (data)=> {
        ...
        updated=await updateLasrtInseted(inserted,data.other,data.user_id);        
